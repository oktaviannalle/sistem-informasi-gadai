<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Tambah Barang Gadai</h2>
    </x-slot>

    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('barang-gadai.store') }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-sm rounded-lg p-6 space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <select name="nasabah_id" class="w-full rounded-lg border-gray-300 text-sm focus:ring-gray-900 focus:border-gray-900">
                        ...
                        @foreach ($nasabahs as $nasabah)
                            <option value="{{ $nasabah->id }}" {{ old('nasabah_id') == $nasabah->id ? 'selected' : '' }}>{{ $nasabah->nama }} ({{ $nasabah->no_ktp }})</option>
                        @endforeach
                    </select>
                    @error('nasabah_id') <p class="text-red-600 text-xs mt-1">{{ $message }}</p> @enderror
                    @if ($nasabahs->isEmpty())
                        <p class="text-xs text-amber-600 mt-1">Belum ada data nasabah. Tambah nasabah dulu sebelum bisa input barang.</p>
                    @endif
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nama Barang</label>
                    <input type="text" name="nama_barang" value="{{ old('nama_barang') }}" class="w-full rounded-lg border-gray-300 text-sm focus:ring-gray-900 focus:border-gray-900">
                ...
                <div>
                    ...
                    <input type="number" name="taksiran_harga" value="{{ old('taksiran_harga') }}" class="w-full rounded-lg border-gray-300 text-sm focus:ring-gray-900 focus:border-gray-900">
